...
export default function PositionChartWorkspace({
    ...
    if (typeof ResizeObserver !== "undefined") {
      resizeObserver = new ResizeObserver(() => {
        chart.applyOptions({
          ...
          height: Math.max(220, container.clientHeight || 420),
        });
      });
  ...
  useEffect(() => {
    ...
    const ma20Data: LineData<number>[] = computeMovingAverage(bars, 20).map(
      (item) => ({
        time: item.time,
        value: item.value,
      }),
    );
    const ma60Data: LineData<number>[] = computeMovingAverage(bars, 60).map(
      (item) => ({
        time: item.time,
        value: item.value,
      }),
    ...
    const mergedMarkers = buildPositionMarkers(
      bars,
      response?.markers || [],
      positionSummary,
    );
  ...
  const latestBar =
    hoveredBar || response?.bars?.[response.bars.length - 1] || null;
  const infoTimeLabel = latestBar
    ? formatDateTime(new Date(latestBar.time * 1000).toISOString())
    : t("common.none");
  const sourceLabel = response
    ? t(sourceLabelKey(response.source))
    : t("trade.positionChartSourceUnavailable");
  ...
  const showGatewayFallbackHint =
    ...
    String(gatewayRuntimeState || "")
      .trim()
      .toLowerCase() !== "healthy";

  return (
    <div
      className={`position-chart-workspace${
        floating
          ? ` is-floating is-placement-${floatingPlacement}`
          : " is-stacked"
      }${dragging ? " is-dragging" : ""}${minimized ? " is-minimized" : ""}`}
      data-testid="position-chart-workspace"
      data-minimized={minimized ? "true" : "false"}
      style={floatingStyle}
    >
      {floating && (
        <div className="position-chart-window-bar">
          <button
            type="button"
            className="position-chart-window-handle"
            data-testid="position-chart-drag-handle"
            onPointerDown={onDragPointerDown}
          >
            {t("trade.positionChartWindowDrag")}
          </button>
          <div className="position-chart-window-bar-spacer" />
          <button
            type="button"
            className="position-chart-window-button"
            data-testid="position-chart-minimize-toggle"
            onClick={onToggleMinimize}
          >
            {minimized
              ? t("trade.positionChartWindowRestore")
              : t("trade.positionChartWindowMinimize")}
          </button>
        </div>
      )}
      <div className="position-chart-header">
        <div>
          <div className="position-chart-kicker">
            {t("trade.positionChartTitle")}
          </div>
        ...
        <div className="position-chart-header-meta">
          ...
          <div className="position-chart-header-meta-row">
            ...
            <strong>
              {response?.meta?.last_bar_at
                ? formatDateTime(response.meta.last_bar_at)
                : t("common.none")}
            </strong>
          </div>
        </div>
      ...
      {!minimized && (
        <>
          <div
            className="position-chart-toolbar"
            data-testid="position-chart-toolbar"
          >
          ...
          {(fallbackActive || showGatewayFallbackHint) && (
            <div
              className="position-chart-status-banner"
              data-testid="position-chart-status-banner"
            >
            ...
            {(loading || statusMessage) && (
              ...
              >
                <div className="position-chart-overlay-card">
                  <strong>
                    {normalizedSelectedSymbol || t("trade.positionChartTitle")}
                  </strong>
      ...
      {floating && !minimized && (
        <div
          className="position-chart-resize-handle"
          data-testid="position-chart-resize-handle"
