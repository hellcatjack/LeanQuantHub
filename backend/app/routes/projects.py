...
def _load_default_config() -> dict[str, Any]:
    ...
    if theme_path.exists():
        try:
            ...
            for item in theme_data.get("categories", []):
                key = str(item.get("key", "")).strip()
                label = str(item.get("label", "")).strip()
                if key:
                    categories.append({"key": key, "label": label or key})
                    theme_items.append(
                        {
                            "key": key,
                            "label": label or key,
    ...
    return {
        "template": "sp500_ai",
        "universe": {"mode": "sp500_history", "include_history": True},
...
def _build_theme_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in theme_items:
        ...
        label = str(item.get("label") or base_item.get("label") or key).strip()
