...
def refresh_bridge(session, *, mode: str, reason: str, force: bool = False) -> dict[str, object]:
    ...
    if script is None:
        ...
        write_refresh_state(root, result="failed", reason="script_missing", message=None)
        return build_bridge_status(root)

    lock = JobLock(_REFRESH_LOCK_KEY, data_root=root)
    if not lock.acquire():
        write_refresh_state(root, result="skipped", reason="lock_busy", message=None)
        return build_bridge_status(root)
    try:
...
def ensure_lean_bridge_live(session, *, mode: str, force: bool = False) -> dict[str, object]:
    status = refresh_bridge(session, mode=mode, reason="auto", force=force)
    return status
