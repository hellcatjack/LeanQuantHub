...
def trading_calendar_config_path(data_root: Path | None = None) -> Path:
    root = data_root or _resolve_data_root()
    return root / "config" / "trading_calendar.json"


def trading_calendar_dir(data_root: Path | None = None) -> Path:
    root = data_root or _resolve_data_root()
    return root / "universe" / "trading_calendar"
...
def trading_calendar_meta_path(data_root: Path | None = None) -> Path:
    root = data_root or _resolve_data_root()
    return trading_calendar_dir(root) / "calendar_meta.json"


def trading_calendar_overrides_path(data_root: Path | None = None) -> Path:
    root = data_root or _resolve_data_root()
    return trading_calendar_dir(root) / "trading_calendar_overrides.csv"
...
def load_trading_calendar_config(data_root: Path | None = None) -> dict[str, Any]:
    root = data_root or _resolve_data_root()
    path = trading_calendar_config_path(root)
    config = _defaults()
    source = "default"
    updated_at = None
    if path.exists():
...
def write_trading_calendar_config(
    payload: dict[str, Any], data_root: Path | None = None
) -> dict[str, Any]:
    root = data_root or _resolve_data_root()
    path = trading_calendar_config_path(root)
    current = load_trading_calendar_config(root)
    config_payload: dict[str, Any] = {
        "source": current.get("source") or DEFAULT_SOURCE,
        "exchange": current.get("exchange") or DEFAULT_EXCHANGE,
        "start_date": current.get("start_date") or DEFAULT_START_DATE,
        "end_date": current.get("end_date") or _default_end_date(),
        "refresh_days": current.get("refresh_days") or DEFAULT_REFRESH_DAYS,
        "override_enabled": current.get("override_enabled", DEFAULT_OVERRIDE_ENABLED),
    }
    for key in ("source", "exchange", "start_date", "end_date"):
        if key in payload and payload[key] is not None:
            config_payload[key] = str(payload[key])
    if "refresh_days" in payload and payload["refresh_days"] is not None:
        config_payload["refresh_days"] = _coerce_int(
            payload["refresh_days"], config_payload["refresh_days"]
        )
    if "override_enabled" in payload and payload["override_enabled"] is not None:
        config_payload["override_enabled"] = _coerce_bool(
            payload["override_enabled"], config_payload["override_enabled"]
        )
    config_payload["updated_at"] = datetime.utcnow().isoformat()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps(config_payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    config = load_trading_calendar_config(root)
    config["config_source"] = "file"
    config["path"] = str(path)
    return config


def load_trading_calendar_meta(data_root: Path | None = None) -> dict[str, Any]:
    root = data_root or _resolve_data_root()
    path = trading_calendar_meta_path(root)
    if not path.exists():
        return {}
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
...
def load_trading_days(
    ...
    if source in {"auto", "local", "exchange_calendars", "lean"}:
        loaded = _load_calendar_days(calendar_path)
        if loaded:
            days = loaded
            calendar_source = meta.get("source") or "local"
            if config.get("override_enabled", True):
                days, overrides_applied = _apply_overrides(days, overrides_path)
            if source == "auto":
                spy_loaded = _load_spy_days()
                if spy_loaded:
                    merged = sorted(set(days).union(spy_loaded))
                    if len(merged) != len(days):
                        days = merged
